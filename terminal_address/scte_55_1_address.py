# Unit Address / MAC Address converters for SCTE 55-1 terminals
# Provides functions that can convert between the Unit address and MAC address
# identifiers for SCTE 55-1 terminals
# See README for more information
# See LICENSE for license

# Pre-computed CRC table
D = []

def mac_to_ua(mac, D=D):
  """Convert MAC address to Unit address"""
  
  # The first 8 bits must be 0 for a valid MAC address
  if mac[0: 2] != "00":
    raise ValueError("Bad MAC address: 40 bits are allowed, but more given")
  
  # Strip the quotes and convert to an array of 1-byte integers
  mac = mac.replace(':', '')
  A = [int(mac[i: i + 2], 16) for i in range(0, 12, 2)]
  
  crc = 0xff
  for i in range(1, 6):
    crc = D[crc ^ A[i]]

  # Convert from hex to decimal and append the CRC
  ua = format(int(mac, 16), "013")
  ua = "{}-{}-{}-{}".format(ua[0: 3], ua[3: 8], ua[8: 13], format(crc, "03"))
  return ua

def ua_to_mac(ua):
  """Convert Unit address to MAC address"""
  n = int(ua.replace("-", "")[:-3])
  if n.bit_length() > 40:
    raise ValueError("Bad Unit address: 40 bits are allowed, but more given")
  
  return format(n, "012x")
